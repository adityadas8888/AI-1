...
#compute function for table move search
def toTree(tabledata):
    returnval="";
    for j in range(7):
        for i in range(6):
            if tabledata[i][j]==0: break
            else: returnval+=str(tabledata[i][j])
        returnval+="|"
    return returnval
...
def evaljhol(boardstate=[]):
    ...
    for i in range(6):
        for j in range(4):
            if [boardstate[i][j],boardstate[i][j+1],boardstate[i][j+2]]==w2:
                return 0

    #check vertical values
    for i in range(7):
        for j in range(4):
            if [boardstate[j][i], boardstate[j+1][i], boardstate[j+2][i]]==w2:
                return 0

    #check diagonal values left to right
    for j in range(3):
        for i in range(4):
            if [boardstate[j][i], boardstate[j+1][i+1], boardstate[j+2][i+2]]==w2:
                return 0
